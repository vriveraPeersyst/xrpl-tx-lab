...
export default function AccountPage() {
  ...
  if (!x.account) return <div className="card space-y-3"><p>Connect your Xaman account to use the testnet wallet: balances, objects and an action for everything you own.</p><button className="btn-primary" onClick={x.connect} disabled={!x.ready || x.connecting}>Connect Xaman</button>{x.error && <p className="text-sm text-danger">{x.error}</p>}<p className="text-xs text-muted">In the Xaman app enable the <b>Testnet</b> network (Settings → Advanced → Network). This site&apos;s payloads are forced to testnet regardless.</p></div>;
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-baseline gap-3"><h1 className="display-lg">My wallet</h1><a className="link font-mono text-sm" href={`${TESTNET_EXPLORER}/accounts/${x.account}`} target="_blank" rel="noreferrer">{x.account} ↗</a><button className="btn-secondary ml-auto" onClick={load} disabled={loading}>{loading ? "Loading…" : "Refresh"}</button></div>
      {err && <p className="text-sm text-danger">{err}</p>}
      {!info ? (
        <div className="card space-y-2"><p>This account <b>does not exist on testnet yet</b>. Request XRP from the faucet to activate it (base reserve: {formatXrp(testnet.reserves.baseXrp)}).</p><button className="btn-primary" disabled={funding} onClick={fund}>{funding ? "Requesting from the faucet…" : "Fund from the testnet faucet"}</button></div>
      ) : (
        <>
          <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="card"><div className="text-xs uppercase text-muted">Faucet</div><button className="btn-secondary mt-1" disabled={funding} onClick={fund}>{funding ? "…" : "+ testnet XRP"}</button></div>
          </section>
          ...
          <section className="card">
            ...
            {objects.length === 0 && <p className="text-sm text-muted">No objects. Every object you create will lock {formatXrp(testnet.reserves.incXrp)} of reserve.</p>}
                ...
                return (
                  <li key={i} className="rounded-[2px] border border-border p-2 text-sm">
                    <div className="flex flex-wrap items-baseline gap-2">
                      ...
                      {r && <span className="badge bg-surface-2 text-muted">{formatXrp(r.units * testnet.reserves.incXrp)} reserve</span>}
