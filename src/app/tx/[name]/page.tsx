    ...
    <div className="space-y-10">
      <header className="space-y-2">
        <div className="flex flex-wrap items-center gap-2 text-xs text-muted">
          <Link href="/tx" className="hover:underline">Transactions</Link><span>/</span><span>{CATEGORIES[spec?.category ?? (pseudo ? "sistema" : "otros")].label}</span>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          {gate ? <AmendmentBadge name={gate.name} /> : <span className="badge bg-surface-2 text-muted">no activation amendment</span>}
      ...
      <section id="builder" className="space-y-3">
        <h2 className="display-md">Build and submit on testnet</h2>
        ...
        <section id="coste" className="space-y-3">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="card">
              ...
              <p className="mt-2 text-xs text-muted">Testnet values: base {formatXrp(testnet.reserves.baseXrp)}, increment {formatXrp(testnet.reserves.incXrp)}. <Link href="/reserves" className="link">Full table and calculator</Link>.</p>
            </div>
          </div>
        </section>
      )}
      ...
      <section className="space-y-3">
        <h2 className="display-md">Fields ({fields.filter((f) => f.inTestnet).length} own + {protocol.commonFields.length} common)</h2>
      ...
      {t.transactor && (
        <section className="space-y-3">
          ...
          <p className="text-xs text-muted">Besides these, any transaction can fail with the generic codes (fee, sequence, signature) from <code>Transactor.cpp</code>: <TerBadge code="telINSUF_FEE_P" /> <TerBadge code="terPRE_SEQ" /> <TerBadge code="tefPAST_SEQ" /> <TerBadge code="tefMAX_LEDGER" /> <TerBadge code="terINSUF_FEE_B" /> <TerBadge code="tefBAD_AUTH" />.</p>
        </section>
      )}
      ...
      <section className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-1 text-sm">
            {amendments.map((a) => <li key={a.name} className="flex items-center gap-2"><AmendmentBadge name={a.name} />{a.gate && <span className="text-xs text-muted">activates the type</span>}</li>)}
