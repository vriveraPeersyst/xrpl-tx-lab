...
export function AmendmentBadge({ name, showName = true }: { name: string; showName?: boolean }) {
  ...
  const cls = state === "activo" ? "bg-accent-soft text-accent-ink" : state === "mayoría" ? "bg-[#edf4ff] text-[#0a4dc0]" : state === "en votación" ? "bg-[#dbf15e] text-black" : state === "vetado" ? "bg-[#fdece7] text-[#a22514]" : "bg-surface-2 text-muted";
  return (
...
export function amendmentState(s?: TestnetAmendment): { label: string; cls: string } {
  ...
  if (s.enabled) return { label: "activo", cls: "bg-accent-soft text-accent-ink" };
  if (s.vetoed) return { label: "vetado", cls: "bg-[#fdece7] text-[#a22514]" };
  if (s.majority) return { label: "mayoría alcanzada", cls: "bg-[#edf4ff] text-[#0a4dc0]" };
  if (s.supported) return { label: "en votación", cls: "bg-[#dbf15e] text-black" };
        ...
        {fields.map((f) => (
          <tr key={f.name} className={!f.inTestnet ? "opacity-60" : ""}>
            ...
            <td className="text-xs text-muted">
              {f.mptSupported && <span className="badge mr-1 bg-accent-soft text-accent-ink">admite MPT</span>}
