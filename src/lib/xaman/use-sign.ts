...
import { getXummInstance, XAMAN_FORCE_NETWORK, type XummCreatedPayload, type XummResolvedPayload } from "./sdk";
  ...
  const sign = useCallback(async (txjson: Record<string, unknown>, instruction?: string) => {
    ...
    try {
      const { created, resolved } = await xumm.payload.createAndSubscribe(
        { txjson, options: { submit: true, force_network: XAMAN_FORCE_NETWORK, expire: 10, instruction: instruction ?? "XRPL Tx Lab · testnet" } },
