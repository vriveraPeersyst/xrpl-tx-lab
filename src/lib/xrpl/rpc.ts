/**
 * Cliente JSON-RPC mínimo contra la XRPL Testnet, usable en navegador.
 * s.altnet.rippletest.net bloquea CORS en navegador; el nodo de XRPL Labs (equipo de Xaman) no.
 */
...
export async function rpc<T = Record<string, unknown>>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  for (const url of TESTNET_RPC_URLS) {
    try {
      ...
      const r = json.result;
      if (r?.status === "error") throw new RpcError(r.error ?? "error", r.error_message ?? r.error_exception ?? r.error ?? "error RPC", r);
      return r;
    } catch (e) {
  ...
  throw lastErr instanceof Error ? lastErr : new Error("RPC no disponible");
}
...
/** Simula una transacción sin enviarla (RPC `simulate`, XLS-69). Devuelve meta y engine_result. */
...
export const dropsToXrp = (drops: string | number) => (Number(drops) / 1_000_000).toLocaleString("es-ES", { maximumFractionDigits: 6 });
